...
namespace istudio
{
    namespace graphic
    {
        ...
        ** @param mesh (istudio::graphic::Mesh): Mesh to use
        **/
        Model::Model(istudio::graphic::Mesh mesh)
        {
            _isLoaded = true;
        ...
        ** @param model (istudio::graphic::Model): Model to use
        **/
        ...
        Model::Model(istudio::graphic::Mesh mesh, std::string filename_text)
        {
            _isLoaded = true;
            _model = LoadModelFromMesh(mesh.getCMesh());
            _boundingBox = GetModelBoundingBox(_model); // BoundingBox(_model);
        ...
        **/
        ...
        void Model::load(istudio::graphic::Mesh mesh, std::string filename_text)
        {
            _isLoaded = true;
            _model = LoadModelFromMesh(mesh.getCMesh());
            _boundingBox = GetModelBoundingBox(_model); // BoundingBox(_model);
        ...
        void Model::load(std::string filename, std::string filename_text)
        {
            _isLoaded = true;
            _model = LoadModel(filename.c_str());
            _model.materials[0].maps[0].texture = LoadTexture(filename_text.c_str());
