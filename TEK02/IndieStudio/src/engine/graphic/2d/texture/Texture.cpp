...
namespace istudio
{
    namespace graphic
    {
        ...
        ** @return Nothing
        **/
        void Texture::draw(istudio::math::Vector2 position, Color tint)
        {
            if (!_isLoaded) return;
        ...
        ** @return Nothing
        **/
        void Texture::draw(istudio::math::Vector2 position, float rotation, float scale, Color tint)
        {
            if (!_isLoaded) return;
            DrawTextureEx(_texture, position.getCVector(), rotation, scale, tint.getCColor());
        }
        
        void Texture::draw(istudio::math::Vector2 position, istudio::math::Rectangle rect, Color tint)
        {
            if (!_isLoaded) return;
            DrawTextureRec(_texture, rect.getCRectangle(), position.getCVector(), tint.getCColor());
        }
        
        void Texture::draw(istudio::math::Vector2 position, istudio::math::Rectangle rect, float scale, Color tint)
        {
            if (!_isLoaded) return;
